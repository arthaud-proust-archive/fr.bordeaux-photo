<form action="{{ $action }}" method="{{ $method }}" enctype="{{ $enctype ?? '' }}">
    ...
    <div class="sm:overflow-hidden">
        ...
        <div class="px-4 py-3 bg-s2 text-right sm:px-6">
            @if($cancel ?? '')
                <a href="{{ $cancel==1?route('dashboard'):$cancel }}" class="inline-flex justify-center py-2 px-4 mr-1 border border-transparent text-sm font-medium rounded-md text-black hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
                    Retour
                </a>
            @endif
