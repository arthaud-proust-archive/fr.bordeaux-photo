...
@switch($type)
    ...
    @case('file')
        <div>
            ...
            <div class="bg-no-repeat bg-contain bg-center dropContainer mt-2 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 @error($id) border-red-500 @enderror border-dashed rounded-md" @if($value ?? '') style="background-image: url({{ $value }})" @endif>
                <div class="space-y-1 text-center px-4 py-2 rounded bg-si bg-opacity-5">
    ...
    @case('select')
        <div class="col-span-6 sm:col-span-3">
            ...
            <select id="{{ $id }}" name="{{ $id }}" autocomplete="false" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-si rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
