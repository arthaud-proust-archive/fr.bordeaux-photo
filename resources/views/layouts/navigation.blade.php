...
$displayableRoutes = [
    ['Évènements','event.index','events'],
    ['À la une','home','info'],
    ['À propos','user.equipe','about'],
]

@endphp

<nav x-data="{ open: false }" class="sticky top-0 bg-s1" style="z-index:9000">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    @foreach($displayableRoutes as $rt)
                    <x-nav-link :href="route($rt[1])" :active="request()->routeIs($rt[1])">
                        {{-- <x-nav.icon :icon="$rt[2]" /> --}}
                        {{ $rt[0] }}
                    </x-nav-link>
                    @endforeach
                </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        ...
                        <x-dropdown-link :href="route('visual.index')">Visuels à poster</x-dropdown-link>
                        @endauthRole
            ...
            <div class="-mr-2 flex items-center sm:hidden">
                @foreach($displayableRoutes as $rt)
                    <x-quick-nav-link :href="route($rt[1])" :active="request()->routeIs($rt[1])">
                        <x-nav.icon :icon="$rt[2]"/>
                    </x-quick-nav-link>
                @endforeach
    ...
    <div id="responsiveMenu" :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            @foreach($displayableRoutes as $rt)
                <x-responsive-nav-link :href="route($rt[1])" :active="request()->routeIs($rt[1])">
                    <x-nav.icon :icon="$rt[2]" class="pr-2"/>
                    {{ $rt[0] }}
            ...
            @authRole('admin')
                ...
                <x-responsive-nav-link :href="route('visual.index')" :active="request()->routeIs('visual.index')">Visuels à poster</x-responsive-nav-link>
            @endauthRole
        </div>
