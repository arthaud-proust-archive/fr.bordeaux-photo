const parseColor = color => color.replace(/rgb\(|\)/g, '').split(/,/g);
...
module.exports = class {
    ...
    fadeFunc(el, attr=false, property, start, end, duration) {
        ...
        let parsedStart = parseColor(start)
        let parsedEnd = parseColor(end)
        ...
        var theInterval = setInterval(function(){
            ...
            if(attr) {
                for(let i=0; i<target.length;i++) {
                    target[i].setAttribute(property, colorname);
                }
            } else {
                for(let i=0; i<target.length;i++) {
                    target[i].style.setProperty(property, colorname);
                }
            }
            u += step_u;
        }, interval);
    ...
    set(theme) {
        ...
        this.updateToggler(theme)
    }

    updateToggler(theme=this._actualTheme) {
        if (theme == 'dark') {
    ...
    load() {
        let oldTransitionDuration = this.transitionDuration;
        this.transitionDuration = 10;
        if (localStorage.getItem(`${this.identifier}-theme`) == undefined) {
            this.set('dark');
        } else {
            this.set(localStorage.getItem(`${this.identifier}-theme`));
        }
    ...
    addCssClasses() {
        ...
        for (const[color, v] of Object.entries(this.themeColors[Object.keys(this.themeColors)[0]])) { // v is value, it's rgb color
            ...
            .focus\\:bg-${name}:focus {background-color: var(${color}) !important;}
            .text-${name} {color: var(${color}) !important;}
            .hover\\:text-${name}:hover {color: var(${color}) !important;}
            .focus\\:text-${name}:focus {color: var(${color}) !important;}
            .border-${name} {border-color: var(${color}) !important;}`;
        }
        document.querySelector('head').appendChild(styles)
    }
