...
var Theme = __webpack_require__(/*! ./theme */ "./resources/js/theme.js");
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.querySelectorAll('a').forEach(function (link) {
    ...
    if (link.getAttribute("href").charAt(0) == '#') {
      ...
      link.addEventListener('click', function (e) {
        e.preventDefault();
        document.body.classList.remove('loaded'); // document.getElementById('maskLeave').classList.add('active');

        setTimeout(function () {
          console.log(link);
          document.location = link.href;
        }, 250); // Cancel the event as stated by the standard.
      });
    }
  ...
  var dropContainers = document.querySelectorAll('.dropContainer');

  var loadFile = function loadFile(file, container) {
    if (!FileReader || !file) {
      return;
    }
    ...
    reader.onload = function (e) {
      container.style.backgroundImage = "url(".concat(e.target.result, ")");
      container.classList.remove('dropStarted');
      container.classList.add('dropDone'); // resizedataURL(e.target.result).then(img=>{
      //     new Compressor(img, {
      //         quality: 0.4,
      //         success(result) {
  ...
  dropContainers.forEach(function (dropContainer) {
    var fileInput = dropContainer.querySelector('input[type="file"]');
    ...
    dropContainer.ondrop = function (e) {
      dropContainer.classList.remove('dragOver');
      dropContainer.classList.add('dropStarted'); // pretty simple -- but not for IE :(
  ...
  var elsDisabled = document.querySelectorAll('[disabled] .dropContainer, [disabled] button, [disabled] a, [disabled] label, [disabled] input, [disabled] textarea, [disabled] select');
  elsDisabled.forEach(function (input) {
  ...
  document.querySelectorAll('.close-alert').forEach(function (alert) {
    setTimeout(function () {
    ...
    alert.addEventListener('click', function (e) {
      ...
      setTimeout(function () {
        _this.closest('.alert').style.display = "none";
      }, 500);
    });
  });
});
...
var parseColor = function parseColor(color) {
  return color.replace(/rgb\(|\)/g, '').split(/,/g);
};

var lerp = function lerp(a, b, u) {
  return (1 - u) * a + u * b;
};

module.exports = /*#__PURE__*/function () {
  function _class(themeColors) {
  ...
  _createClass(_class, [{
    ...
    value: function fadeFunc(el) {
      ...
      var u = 0.0;
      var parsedStart = parseColor(start);
      var parsedEnd = parseColor(end);
      var theInterval = setInterval(function () {
        if (u >= 1.0) {
          clearInterval(theInterval);
        }

        var r = parseInt(lerp(parsedStart[0], parsedEnd[0], u));
        var g = parseInt(lerp(parsedStart[1], parsedEnd[1], u));
        var b = parseInt(lerp(parsedStart[2], parsedEnd[2], u));
        var colorname = 'rgb(' + r + ',' + g + ',' + b + ')'; // el.style.setProperty(property, colorname);
    ...
    key: "fade",
    value: function fade(theme) {
      for (var _i = 0, _Object$entries = Object.entries(this.colors[theme]); _i < _Object$entries.length; _i++) {
    ...
    value: function set(theme) {
      this._actualTheme = theme;
      this.fade(theme); // $('html').attr('data-theme',theme);

      localStorage.setItem("".concat(this.identifier, "-theme"), theme);
    ...
    key: "addCssClasses",
    value: function addCssClasses() {
      var styles = document.createElement("style");

      for (var _i4 = 0, _Object$entries4 = Object.entries(this.colors[Object.keys(this.colors)[0]]); _i4 < _Object$entries4.length; _i4++) {
        var _Object$entries4$_i = _slicedToArray(_Object$entries4[_i4], 2),
            color = _Object$entries4$_i[0],
            v = _Object$entries4$_i[1];

        // v is value, it's rgb color
        var name = color.replace(/-/g, '');
        styles.innerHTML += ".bg-".concat(name, " {background-color: var(").concat(color, ") !important;}\n            .hover\\:bg-").concat(name, ":hover {background-color: var(").concat(color, ") !important;}\n            .focus\\:bg-").concat(name, ":focus {background-color: var(").concat(color, ") !important;}\n            .text-").concat(name, " {color: var(").concat(color, ") !important;}\n            .hover\\:text-").concat(name, ":hover {color: var(").concat(color, ") !important;}\n            .focus\\:text-").concat(name, ":focus {color: var(").concat(color, ") !important;}\n            .border-").concat(name, " {border-color: var(").concat(color, ") !important;}");
      }
